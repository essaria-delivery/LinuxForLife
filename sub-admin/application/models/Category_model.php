...
class Category_model extends CI_Model{
        ...
        public function get_store_categories()
        {
            ...
            //start mine show categ based on store_id_login
             $q = $this->db->query("SELECT a.*, Deriv1.prtitle FROM `categories` a  LEFT OUTER JOIN (SELECT `id`, `title` as `prtitle` FROM `categories`) as Deriv1 ON Deriv1.`id` = a.`parent` where a.store_id_login =". _get_current_user_id($this));
        //           $this->db->select('*');
        //   $this->db->from('categories');
        //   $this->db->where('store_id_login', _get_current_user_id($this));
          // $query = $this->db->get();
           return $q->result();
            //end  mine show categ based on store_id_login
