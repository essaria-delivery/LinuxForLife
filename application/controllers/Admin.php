...
class Admin extends MY_Controller {
    ...
    public function dashboard(){
        if($this->session->userdata('admin_data')){
            ...
            $date = date("Y-m-d");
            $store="";
            $data["today_orders"] = $this->product_model->get_sale_orders(" and sale.on_date = '".$date."' ", $store);
             $nexday = date('Y-m-d', strtotime(' +1 day'));
            $data["nextday_orders"] = $this->product_model->get_sale_orders(" and sale.on_date = '".$nexday."' ", $store);
    ...
    public function orders($store_id){
        if($this->session->userdata('admin_data')){
            ...
            $data['date_range_lable'] = $this->input->post('date_range_lable');
             ...
             $filter = "and sale.new_store_id='".$store_id."'";
        ...
        if($this->session->userdata('admin_data')){
            ...
            if(!empty($order)){
                ...
                $this->session->set_flashdata("message",'<div class="alert alert-success alert-dismissible" role="alert">
                    ...
                    //$q = $this->db->query("Select * from `registers` where `user_id`=20");
                    //$user = $q->row();
                    // $row=$q->row();
                    ...
                    $message="Assign Thanks for being with Us";
...
function edit_product($prod_id){
       if($this->session->userdata('admin_data')){
             ...
             $ct1  = $this->db->query("select * from products where product_id='$prod_id'");
            $data["products"] = $ct1->row();
            // print_r($data);
            $this->load->view("admin/product/update",$data);
            if(isset($_POST['edit']))
                {
                   $name=$_POST["prod_title"];
                   $prod_id=$_POST["id"];
                     
                     $ct2  = $this->db->query("update  products set product_name='$name' where product_id='$prod_id'");
                     
                     
                    //  $ctt = $this->db->query("select * from products");
                    // $data["products"] = $ctt->result();
                    // $this->load->view("admin/product/addd2",$data);   
                ...
                {
                $products->product_id;
                 $ct3  = $this->db->query("delete from products where product_id='$prod_id'");
                 redirect('admin/add_product');
                }
        }
        else
...
//start delete operation by mine
function edit_productt($prod_id){
       if($this->session->userdata('admin_data')){
           $ct1  = $this->db->query("select * from products where product_id='$prod_id'");
            $data["products"] = $ct1->row();
             print_r($data);
                    ...
                    $ct2  = $this->db->query("delete from products where product_id='$prod_id'");
                    //  $ctt = $this->db->query("select * from products");
                    // $data["products"] = $ctt->result();
                    // $this->load->view("admin/product/addd2",$data);   
                    redirect('admin/add_product');
       ...
       if($this->session->userdata('admin_data')){
            ...
            $this->load->library('form_validation');
                ...
                else
                {
                    ...
                    if(!empty($_FILES['prod_gallery']['name'])){

                        $filesCount = count($_FILES['prod_gallery']['name']);
                
                        for($i = 0; $i < $filesCount; $i++){
    
                            $_FILES['file']['name']     = $_FILES['prod_gallery']['name'][$i];
                            $_FILES['file']['type']     = $_FILES['prod_gallery']['type'][$i];
                            $_FILES['file']['tmp_name'] = $_FILES['prod_gallery']['tmp_name'][$i];
                            $_FILES['file']['error']    = $_FILES['prod_gallery']['error'][$i];
                            $_FILES['file']['size']     = $_FILES['prod_gallery']['size'][$i];
                            
                            if($_FILES["file"]["size"] > 0){
                                $con['upload_path']          = './uploads/products/';
                                ...
                                else
                                {
                                    ...
                                    $data_img=array(
                                        'product_id'=>'',
                                        'image'=>'uploads/products/'.$_FILES['file']['name'],
                                        'image_type'=>$_FILES['file']['type']
                                    );
                                    
                                    $done = $this->db->insert('products_images', $data_img);
                                    if($done)
                                    {
                                        $m=1;
                                    }
        ...
        if(_is_user_login($this))
        {
                ...
                if(isset($_POST['addcatg']))
                {
                    ...
                    $this->load->model("common_model");
                        ...
                        $prod_tittle=$this->input->post("prod_title");
                        if($prod_tittle!="")
                        {
                            $in_id = $this->common_model->data_insert("products",$array);
                              if($in_id)
                              {
                              $this->session->set_flashdata("message",'<div id="abc" class="alert alert-success alert-dismissible" role="alert">
                                        <i class="fa fa-check"></i>
                                      <button type="button" class="close" data-dismiss="alert"  onclick="a()"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
                                      <strong>Success!</strong>successfull...."'.$in_id.'"
                                    </div>');
                        ...
                        $this->session->set_flashdata("message",'<div id="abc" class="alert alert-success alert-dismissible" role="alert">
                                        <i class="fa fa-check"></i>
                                      <button type="button" class="close" data-dismiss="alert" onclick="a()"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
                                      <strong>wrong!</strong>enter product name....
                                    </div>');    
                        }
                ...
                $ct  = $this->db->query("select * from products");
            $data["products"] = $ct->result();
            $this->load->view("admin/product/addd2",$data);
        }
    else
        {
...
/* ========== End page page setting ========*/
   function testnoti(){
                      ...
                      $this->session->set_flashdata("message", '<div class="alert alert-warning alert-dismissible" role="alert">
                            ...
                            foreach($registers as $regs){
                                if($regs->user_ios_token!=""){
                                    ...
                                    $result = $this->send_notification($regs->user_ios_token, $message,"android");
                                }
                            }
    ...
    $this->load->view('api/template/account2');
     }
    ...
    public function all_attributes(){
        if($this->session->userdata('admin_data')){
            $q=$this->db->query("SELECT * FROM `attribute_tbl`");
            $data['allatt']=$q->result();
            $this->load->view("admin/attribute/all_attribute",$data);
        }
        else
        {
            redirect('admin');
     ...
     public function sms(){
	     if($this->session->userdata('admin_data')){
	         if(isset($_POST['alert']))
	         {
                ...
                $msg_register=($this->input->post("reg_sms")=="on")?1:0; 
                $mail_register=($this->input->post("reg_mail")=="on")?1:0;
                   ...
                   // "rewards"=>$this->input->post("rewards")
                    ...
                    $check=$this->common_model->data_update("message",$array,array("store_id"=>0));
                    $this->session->set_flashdata("message",'<div class="alert alert-success alert-dismissible" role="alert">
                                        <i class="fa fa-check"></i>
                                      <button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
                                      <strong>Success!</strong> Your request added successfully...
            ...
            if(isset($_REQUEST["update"]))
            {
                ...
                else
                {
                    $this->session->set_flashdata("success_req",'<div class="alert alert-success alert-dismissible" role="alert">
                                      ...
                                      <button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
                                      <strong>Success!</strong> Update Successfull </div>');
                    redirect('admin/sms_new');
                }
            }
           //$this->load->view('admin/setting/language',$data);
            // end lang
           
            $this->load->view("admin/sms/club",$data); 
            //$this->load->view("admin/sms/sms_alert",$data); 
    ...
    {
        $this->load->view('api/template/account');
    }
    
    public function add_app_home_banner()
    {
       if($this->session->userdata('admin_data')){
            ...
            if(isset($_REQUEST["addslider"]))
            {
                ...
                else
                {
                    $add = array(
                       ...
                       $this->db->insert("app_dashboard_banner",$add);
                    ...
                    redirect('admin/list_app_home_banner');
                }
            }
        $this->load->view('admin/home_banner/addslider',$data);
        }
        else
        {
            redirect('admin');
    ...
    public function list_app_home_banner()
    {
        if($this->session->userdata('admin_data')){
        //   $data["error"] = "";
        //   $data["active"] = "listslider";
        ...
        $allslider = $this->db->get();
        $data['slider']=$allslider->result();
        // var_dump($data["allslider"]->result());
        $this->load->view('admin/home_banner/listslider',$data);
        }
        else
        {
            redirect('admin');
    ...
    public function edit_app_home_banner($id)
    {
       if($this->session->userdata('admin_data'))
       {
            ...
            if(isset($_REQUEST["saveslider"]))
            {
                ...
                {
                    $this->load->model("slider_model");
                    $this->slider_model->edit_home_slider($id); 
                    $this->session->set_flashdata("success_req",'<div class="alert alert-success alert-dismissible" role="alert">
                                        <i class="fa fa-check"></i>
                                      <button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
                                      <strong>Success!</strong> Your Slider saved successfully...
           ...
           $this->load->view('admin/home_banner/editslider',$data);
        }
        else
        {
            redirect('admin');
    ...
    public function delete_app_home_banner($id){
        ...
        if($this->session->userdata('admin_data'))
        {
            ...
            $this->db->where('id',$id);
            $allslider = $this->db->get();
            $slider=$allslider->row();
            if($slider){
                unlink("uploads/sliders/".$slider->slider_image);
                $this->db->query("Delete from app_dashboard_banner where id = '".$slider->id."'");
                redirect("admin/list_app_home_banner");
           }
        }
        else
