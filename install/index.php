...
$myfile = fopen("../sub-admin/application/config/database.php", "w") or die("Unable to open file!");
$txt = "";
fwrite($myfile, $txt);
$txt = '<?php
...
$config['base_url'] = ((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] == "on") ? "https" : "http");
        ...
        $config['base_url'] .= str_replace(basename($_SERVER['SCRIPT_NAME']),"",$_SERVER['SCRIPT_NAME']);
        $base = $config['base_url'];
        $base = str_replace("/install/","/sub-admin/",$base);

file_put_contents("../sub-admin/application/config/config.php",'
...
$config["base_url"] = "'.$base.'";', FILE_APPEND | LOCK_EX);
  ...
  <div class="content has-text-centered">
    <p>
      Copyright <?php echo date('Y'); ?> GoFresh, All rights reserved.
    </p>
    <br>
  </div>
</body>
