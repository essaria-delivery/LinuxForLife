...
/* user login json */
    ...
    {   
        ...
        // start sms
                $getAuthKey = "197064AvhL4Byh5bc4613f";
                $getSenderId = "OTPTEC";
               // $getInvitationMsg = "Your Registration Successful"; 

                $authKey = $getAuthKey;
              //  $mobileNumber = 8859593839;
        ...
        function all_products(){
            ...
            $this->load->model("product_model");
            $user_id=$this->input->post("user_id");
            $data = $this->product_model->all_products_list($user_id);
...
public function add_purchase(){
 ...
 function get_leftstock(){
        ...
        left outer join(select SUM(qty) as c_qty,product_id from sale_items group by product_id) as consuption on consuption.product_id = products.product_id 
            ...
            where products.store_id_login=".$user_id. " ORDER BY stock ASC");
        $data = $q->result();
echo json_encode($data);
      }
 ...
 /* end about us */         
    ...
    public function update_location(){
            $data = array(); 
                ...
                //"pincode"=>$this->input->post("user_id"),
                    $insert_array=  array(
                                            "user_lat"=>$this->input->post("lat"),
                                            "user_lng"=>$this->input->post("lng")
                                            );
                     
                    $this->load->model("common_model");
                     
                    
                   $this->common_model->data_update("delivery_boy",$insert_array,array("id"=>$this->input->post("user_id")));
