...
class LicenseBoxAPI {
  ...
  { 
    $this->product_id = '180CCA37';
    $this->api_url = 'https://thecodecafe.in/activator/';
    $this->current_version = 'v1.0.0';
    $this->current_path = dirname(_FILE_);  //$this->current_path = realpath(_DIR_);
  ...
  public function verify_license($time_based_check = false, $license = false, $client = false){
    ...
    $today = date('d-m-Y');
    if(empty($_SESSION["bd84236aed"])){
      $_SESSION["bd84236aed"] = '0000-00-00';
    }
    if($type==1){
    if($today>=$_SESSION["bd84236aed"]){
